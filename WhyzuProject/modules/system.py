...
#
# t.me/SharingUserbot & t.me/suportsipuserbot
...
from WhyzuProject import BOTLOG_CHATID, LOGGER
from WhyzuProject.helpers.basic import edit_or_reply
from WhyzuProject.helpers.misc import HAPP
...
async def shutdown_bot(client: Client, message: Message):
    if BOTLOG_CHATID:
        await client.send_message(
            BOTLOG_CHATID,
            "**#SHUTDOWN** \n"
            "**pyroNande-userbot** telah di matikan!\nJika ingin menghidupkan kembali silahkan buka heroku",
        )
    await edit_or_reply(message, "**pyroNande-userbot Berhasil di matikan!**")
    if HAPP is not None:
...
async def logs_ubot(client: Client, message: Message):
    ...
    await client.send_document(
        message.chat.id,
        "Logs-Heroku.txt",
        thumb="WhyzuProject/resources/logo.jpg",
